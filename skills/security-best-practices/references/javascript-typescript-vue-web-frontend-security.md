...
Read `_common-web-security-spec.md` first. This file adds Vue-specific audit order, untrusted inputs, frontend state-changing action notes, production baseline, rules, detection hints, and fixes.

Frontend trust model: browser-shipped code is attacker-readable/modifiable. Secrets and security enforcement cannot rely on frontend-only logic.
