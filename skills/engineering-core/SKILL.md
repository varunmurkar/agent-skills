...
4. Load `../testing/SKILL.md` when writing new tests, debugging failures, or reducing suite/runtime cost.
5. Load `references/documentation-core.md` when producing or updating documentation.
6. Load `references/error-handling-core.md` when defining or revising failure behavior.
7. Load one of the feature lifecycle references based on task type:
...
8. Load `references/analytics-monitoring-core.md` only when telemetry/observability is touched.
9. Load `references/tenant-isolation-conditional.md` only when tenant guidance is enabled.
10. Load `references/compliance-conditional.md` only when compliance guidance is enabled.
11. If the task includes DB/schema/migration/query design on Postgres, require loading `../supabase-postgres-best-practices/SKILL.md` before finalizing implementation decisions.
